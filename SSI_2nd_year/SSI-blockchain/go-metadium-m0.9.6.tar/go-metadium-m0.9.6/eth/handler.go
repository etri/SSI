...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == GetBlockBodiesMsg:
		...
		go func() error {
			for _, hash = range hashes {
				if bytes >= softResponseLimit || len(bodies) >= downloader.MaxBlockFetch {
					break
				}
	...
	case msg.Code == BlockBodiesMsg:
		...
		go func() error {
			...
			if len(transactions) > 0 {
				signer := types.MakeSigner(pm.blockchain.Config(), pm.blockchain.CurrentBlock().Number())
	...
	case p.version >= eth63 && msg.Code == GetNodeDataMsg:
		...
		go func() error {
			for _, hash = range hashes {
				if bytes >= softResponseLimit || len(data) >= downloader.MaxStateFetch {
					break
				}
