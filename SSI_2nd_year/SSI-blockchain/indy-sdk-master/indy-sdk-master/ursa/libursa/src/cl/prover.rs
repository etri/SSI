...
impl Prover {
    ...
    ///
    /// let _master_secret = Prover::new_master_secret(master_secret_id: &str).unwrap();
    /// ```
    pub fn new_master_secret(master_secret_id: &str) -> UrsaCryptoResult<MasterSecret> {
        Ok(MasterSecret {
            // ms: bn_rand(LARGE_MASTER_SECRET)?,
            ms: string_to_bignumber(master_secret_id)?,
        })
    }
...
mod tests {
    ...
    fn generate_master_secret_works() {
        ...
        let ms = Prover::new_master_secret("master_secret").unwrap();
        assert_eq!(
            ms.ms.to_dec().unwrap(),
