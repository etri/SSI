...
import static com.iitp.iitp_demo.Constants.response_url;
...
public class RequestWebVpActivity extends BaseActivity{
...
//        setActionBarSet(layout.toolbar, getString(R.string.vc_request), false);
        ...
        new Thread(() -> {
            try{
                if(vcList.size() == 0){
                    runOnUiThread(() -> {
                        ToastUtils.custom(Toast.makeText(RequestWebVpActivity.this, "VC가 없습니다. ", Toast.LENGTH_SHORT)).show();
                        setResult(RESULT_CANCELED);
                        finish();
                    });
                }else{
    ...
    private void pinCode(){
        ...
        if((BiometricUtils.getPincode(RequestWebVpActivity.this) == null && !(BiometricUtils.isFingerPrint(RequestWebVpActivity.this)))){
            ...
            if(BiometricUtils.isFingerPrint(RequestWebVpActivity.this)){
                fingerBiofactory(this);
            }else{
                new Handler().postDelayed(new Runnable(){
                    @Override
...
//                requested_attributes = true;
    ...
    private void makeDelegatedVC(String did, Map<String, VpRequestZKPDelegateDataVo> delegateAttributes, Map<String, VpRequestZKPReferentDataVo> requestAttributes){
        ...
        String deleted_vc = creator.vcCreateDelegationToken(RequestWebVpActivity.this, did, payment, did_delegator, delegator_attr);
    ...
    private void savePreVCList(String key, String jwt){
        ...
        CommonPreference.getInstance(RequestWebVpActivity.this).getSecureSharedPreferences().edit().putString(key, jwt).apply();
    }
    ...
    private String getDID(){
        ...
        String json = CommonPreference.getInstance(RequestWebVpActivity.this).getSecureSharedPreferences().getString(DID_LIST, null);
        if(json != null){
    ...
    private String makeVpData(String did, List<String> vcList){
        ...
        String keyid = keyManager.getManagementKeyId(RequestWebVpActivity.this);
        ECPrivateKey privatKey = ECKeyUtils.toECPrivateKey(keyManager.getPrivate(RequestWebVpActivity.this), "secp256k1"); // PrivateKey load
    ...
    private List<String> getVcList(Boolean delegator, Map<String, VpRequestZKPReferentDataVo> requestAttributes){
        ...
        if(delegator){
            String vc = CommonPreference.getInstance(RequestWebVpActivity.this).getSecureSharedPreferences().getString(NoneZKPStoreDelegationToken, null);
            if(vc != null){
                vclist.add(vc);
            }
        }
                    ...
                    if(key1.equals("type")){
                        ...
                        if(preKey != null){
                            ...
                            if(preKey.equals("ProductCredential") || preKey.equals("ProductProofCredential")){
                                ...
                                String vc = CommonPreference.getInstance(RequestWebVpActivity.this).getSecureSharedPreferences().getString(preKey, null);
            ...
            for(String key : claims.keySet()){
                ...
                if(types.get(1).equals("IdentificationCredential")){
                    runOnUiThread(() -> {
                        ...
                        layout.textView.setText("신분증 VC");
    ...
    private void startPinCodeActivity(){
        Intent intent;
        intent = new Intent(RequestWebVpActivity.this, PincodeActivity.class);
        intent.putExtra(PIN_SETTING_TYPE, 2);
    ...
    private void requestCredential(String url, String vp){
        ...
        rtn.enqueue(new Callback<ResponseBody>(){
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t){
                ...
                ToastUtils.custom(Toast.makeText(RequestWebVpActivity.this, "Server error", Toast.LENGTH_SHORT)).show();
                layout.progresslayout.setVisibility(View.GONE);
            }
        });
    }
    ...
    private void fingerPrintFactory(){
        BiometricDialog biometricDialog = new BiometricDialog(RequestWebVpActivity.this,
                ...
                new BiometricDialog.OnAuthenticationListener(){
                    ...
                    public void onSuccess(FingerprintManager.AuthenticationResult result){
                        BiometricUtils.enableFingerPrint(RequestWebVpActivity.this);
                        new Handler().postDelayed(new Runnable(){
                            @Override
                            public void run(){
